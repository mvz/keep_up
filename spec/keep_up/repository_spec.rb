...
describe KeepUp::Repository do
  describe '#updated_dependency_for' do
    ...
    context 'when only regular releases are available' do
      ...
      context 'when a current pre-release cannot be updated' do
    ...
    context 'when pre-releases are also available' do
      ...
      context 'when a current pre-release cannot be updated' do
        let(:locked_version_string) { '1.2.0.pre.2' }
    ...
    context 'when only pre-releases are available' do
      let(:version_strings) { ['1.2.0.pre.1', '1.2.0.pre.2'] }
